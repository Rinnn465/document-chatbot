...
namespace DocumentChatbot.Core.Application.Services;
    ...
    TimeProvider timeProvider) : IChatService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return new AskQuestionResult(session.Id, userMessage, assistantMessage, isGrounded);
    }
    ...
    private static void EnsureOwner(ChatSession session, string userId)
    {
        var normalizedUserId = NormalizeRequired(userId, nameof(userId));
        if (!string.Equals(session.UserId, normalizedUserId, StringComparison.Ordinal))
        {
